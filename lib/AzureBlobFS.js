...
class AzureBlobFS {
  ...
  stat(pathname, options = { snapshot: false }) {
    ...
    if (pathname) {
      ...
      return this._blobServicePromised.getBlobProperties(this.container, pathname, getOptions)
        .then(blobProperties => {
          const snapshots = [];
          let continuationToken;
          ...
          return (
            doWhilst(
              ...
              const stat = {
                isDirectory: IS_FILE,
                mode: R_OK | W_OK,
                mtime: new Date(blobProperties.lastModified),
