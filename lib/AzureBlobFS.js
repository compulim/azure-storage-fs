...
const parseSASToken = require('./util/parseSASToken');
...
class AzureBlobFS {
  constructor(account, secretOrSAS, container, options = DEFAULT_OPTIONS) {
    this.options = options;

    const secret = ~secretOrSAS.indexOf('&') ? null : secretOrSAS;
    const sas = ~secretOrSAS.indexOf('&') ? parseSASToken(secretOrSAS) : null;

    this._blobService = require('azure-storage').createBlobService(account, secret, null, sas);
  ...
  sas(pathname, options = DEFAULT_SAS_OPTIONS) {
    ...
    if (
      ...
      && !(options.expiry instanceof Date)
    ) {
      throw new Error('expiry must be unset or set to a number or Date');
    }
