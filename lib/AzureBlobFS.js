...
class AzureBlobFS {
  constructor(account, secret, container, options = DEFAULT_OPTIONS) {
    if (account instanceof BlobService) {
  ...
  async rename(oldPathname, newPathname) {
    ...
    const result = await this._blobServicePromised.startCopyBlob(
      oldURI,
      this.container,
      newPathname,
      {}
    ...
    try {
      ...
      while (copyStatus !== 'success') {
        console.log(`watching copy status: ${ copyStatus }`);
    ...
    if (pathname) {
      ...
      try {
        const [properties, metadata, snapshots] = await Promise.all([
          ...
          options.metadata ? this._blobServicePromised.getBlobMetadata(this.container, pathname, getOptions).then(result => result.metadata) : null,
        ...
        const result = await this._blobServicePromised.listBlobDirectoriesSegmentedWithPrefix(this.container, pathname, null, { maxResults: 1 });
